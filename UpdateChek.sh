...
VERSION=${Version}
[ -z "$VERSION" ] && VERSION=0
if [ ! -z "$(curl -s https://raw.githubusercontent.com/bclrpd/Pizarra/main/Current | grep $SERIAL)" ] ; then
	LINE=$(curl -s https://raw.githubusercontent.com/bclrpd/Pizarra/main/Current | grep $SERIAL)
	NEW_VERSION=${LINE##*=}
	BANCA=${LINE%%=*}
	T_BANCA=${BANCA%%_*}
	N_BANCA=${BANCA##*_}
	...
	elif [ $NEW_VERSION -lt $VERSION ] ; then
		echo "Version=$NEW_VERSION" > Current.ini && echo "Banca=$N_BANCA" >> Current.ini && echo "Tipo=$T_BANCA" >> Current.ini
	fi
elif [ ! -z "$(curl -s https://raw.githubusercontent.com/bclrpd/Pizarra/main/Current | grep 0123456789)" ] ; then
    LINE=$(curl -s https://raw.githubusercontent.com/bclrpd/Pizarra/main/Current | grep 0123456789)
	NEW_VERSION=${LINE##*=}
	BANCA=${LINE%%=*}
	T_BANCA=${BANCA%%_*}
	N_BANCA=${BANCA##*_}
